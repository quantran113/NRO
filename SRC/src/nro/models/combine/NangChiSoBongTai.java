...
public class NangChiSoBongTai {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (bongTai != null && honBongTai != null && daXanhLam != null) {
                ...
                int currentHonSoLuong = honBongTai.quantity;
                Item bagHon = InventoryService.gI().findItemBag(player, HON_BONG_TAI_ID);
                if (bagHon != null) {
                    currentHonSoLuong = Math.max(currentHonSoLuong, bagHon.quantity);
                }
                ...
                } else if (currentHonSoLuong < REQUIRED_HON_BONG_TAI) {
    ...
    public static void nangChiSoBongTai(Player player) {
        try {
            ...
            int currentHonSoLuong = honBongTai != null ? honBongTai.quantity : 0;
            Item bagHon = InventoryService.gI().findItemBag(player, HON_BONG_TAI_ID);
            if (bagHon != null) {
                currentHonSoLuong = Math.max(currentHonSoLuong, bagHon.quantity);
            }

            if (currentHonSoLuong < REQUIRED_HON_BONG_TAI || daXanhLam == null || daXanhLam.quantity < 1) {
            ...
            Item bongTai = player.combineNew.itemsCombine.stream()
                    .filter(item -> item != null && item.isNotNullItem() && item.template != null && item.template.id == BONG_TAI_ID)
                    .findFirst()
                    .orElse(null);

            if (bongTai == null) {
                Service.gI().sendThongBao(player, "Thiếu Bông tai Porata cấp 2.");
                return;
            }
